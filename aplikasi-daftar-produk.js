...
const aplikasiDaftarProduk = {
    ...
    menampilkanDaftarProduk: function () {
        ...
        this.daftarProduk.forEach((produk, index) => {
            componentDaftarProduk.innerHTML += `${produk.nama} <br> ${produk.harga} <br> ${produk.stok} <br> <img src="${produk.gambar}" width="200" height="200";> <button onclick="aplikasiDaftarProduk.editProduk(${index})">Edit</button> <button onclick="aplikasiDaftarProduk.hapusProduk(${index})">Hapus</button>`;
        });
    },
